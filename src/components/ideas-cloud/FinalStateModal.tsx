...
interface FinalStateModalProps {
  idea: any;
  onClose: () => void;
}

export default function FinalStateModal({ idea, onClose }: FinalStateModalProps) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/v1/ideas/${idea.id}/final-state`);
  ...
  const handleSaveManual = async () => {
    setSaving(true);
    try {
      await fetch(`/api/v1/ideas/${idea.id}/final-state`, {
        method: 'POST',
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-card-bg rounded-2xl shadow-2xl p-6 w-full max-w-4xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <div className="min-w-0">
            <h2 className="text-lg font-semibold text-foreground">Final State</h2>
            <p className="text-xs text-muted truncate max-w-[500px]">{idea.text}</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              <textarea
                ...
                placeholder="Describe your intended final state for this idea..."
              ...
              <div className="w-full h-64 px-3 py-2 border border-border rounded-xl text-sm overflow-y-auto bg-gray-50">
                {generated ? (
                  ...
                  <p className="text-muted italic">
                    No AI prediction yet. Click Generate to create one.
                  </p>
                )}
              </div>
              <button
